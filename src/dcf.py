...
def coletar_fluxo_caixa_livre(ticker, anos=5):
    try:
        ...
        # Estimar FCF manualmente se campo não estiver presente
        if 'Free Cash Flow' in cf.index:
            fcf_series = cf.loc['Free Cash Flow']
        elif ('Total Cash From Operating Activities' in cf.index and
              'Capital Expenditures' in cf.index):
            fcf_series = cf.loc['Total Cash From Operating Activities'] + cf.loc['Capital Expenditures']
        else:
            raise ValueError("Não foi possível estimar o fluxo de caixa livre para este ticker.")

        # Usa os últimos 4 trimestres como base
